...
def summ_matrix(m1,m2, x, y):#сложение 2х матриц одного размера
    matrix_3 = [[0 for i in range(x)] for i in range(y) ]
    for i in range(y):
        for j in range(x):
            matrix_3[i][j] = m1[i][j] + m2[i][j]
    return matrix_3

def matrix(t): #вывод матрицы в читабельном виде
    for i in t:
        print(*i)
def generate(m,x,y): #генерировать случайные значения в матрице
    gen_min, gen_max = list(map(int, input(f"Укажите через запятую от какого до какого числа генерировать случайные значения в матрице  ").split(',')))
    for i in range(y):
        for j in range(x):
            m[i][j] = random.randint(gen_min, gen_max)            


x, y = list(map(int, input(f"Укажите какой размерности генерировать матрицы через запятую x,y  ").split(',')))
matrix_1 = [[0 for i in range(x)] for i in range(y)]
matrix_2 = [[0 for i in range(x)] for i in range(y)]

generate(matrix_1,x,y)
generate(matrix_2,x,y)


print('matrix_1')
matrix(matrix_1) #Вывод matrix_1
print('matrix_2')
matrix(matrix_2) #Вывод matrix_2
print('сумма матриц matrix_3')

matrix(summ_matrix(matrix_1,matrix_2, x, y)) #Вывод matrix_3 Сумма matrix_1 и matrix_2
